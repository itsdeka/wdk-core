...
var chunkFI6Q4OWN_cjs = require('./chunk-FI6Q4OWN.cjs');
...
var OftBridgeBase = class {
  ...
  validateInput(input) {
    ...
    uiCore.assert(srcDeployment.token.equals(input.srcToken), "Invalid srcToken");
    uiCore.assert(dstDeployment.token.equals(input.dstToken), "Invalid dstTOken");
  }
  ...
  async getAllowance({ token, address, dstChainKey }) {
    ...
    if (dstChainKey && !uiCore.isEvmChainKey(dstChainKey) && oftNative) {
      oft = oftNative;
    } else {
      oft = oftProxy;
      approvalRequired = oftProxy?.approvalRequired !== false;
    }
    const skipApproval = !oft || !uiCore.hasAddress(token) || approvalRequired === false;
    if (skipApproval) {
  ...
  async approve({ amount, dstChainKey }) {
    ...
    uiCore.assert(oft, "No oftProxy or oftNative");
  ...
  tryGetDeployment(chainKey) {
    return chunkCUSC3AWA_cjs.tryGetDeployment(chainKey, this.config);
  }
  getDeployment(chainKey) {
    return chunkCUSC3AWA_cjs.getDeployment(chainKey, this.config);
  }
  getDefaultExtraGas(srcChainKey, dstChainKey) {
    return dstChainKey === "arbitrum" ? 3e6 : uiCore.isAptosChainKey(dstChainKey) ? 1e4 : (
      // other evm
      25e4
    );
  }
...
chunkFI6Q4OWN_cjs.__publicField(OFTV0__factory, "abi", _abi);
var _abi2 = [
  {
    inputs: [
      {
...
chunkFI6Q4OWN_cjs.__publicField(Endpoint__factory, "abi", _abi2);
var _abi3 = [
  {
    inputs: [
      {
...
chunkFI6Q4OWN_cjs.__publicField(OFTV1__factory, "abi", _abi3);
var _abi4 = [
  {
    inputs: [
      {
...
chunkFI6Q4OWN_cjs.__publicField(OFTV2__factory, "abi", _abi4);
var _abi5 = [
  {
    inputs: [
      {
...
chunkFI6Q4OWN_cjs.__publicField(OFTV2Fee__factory, "abi", _abi5);
var _abi6 = [
  {
    inputs: [
      {
...
chunkFI6Q4OWN_cjs.__publicField(OFTV3__factory, "abi", _abi6);
var _abi7 = [
  {
    inputs: [
      {
...
chunkFI6Q4OWN_cjs.__publicField(OFTV3_Adapter__factory, "abi", _abi7);
var _abi8 = [
  {
    inputs: [
      {
...
chunkFI6Q4OWN_cjs.__publicField(ProxyOFTV1__factory, "abi", _abi8);
var _abi9 = [
  {
    inputs: [
      {
...
chunkFI6Q4OWN_cjs.__publicField(ProxyOFTV2__factory, "abi", _abi9);
var _abi10 = [
  {
    inputs: [
      {
...
var ProxyOFTV2Fee__factory = class {
  static createInterface() {
    return new ethers.utils.Interface(_abi10);
  }
  static connect(address, signerOrProvider) {
    return new ethers.Contract(address, _abi10, signerOrProvider);
  }
};
chunkFI6Q4OWN_cjs.__publicField(ProxyOFTV2Fee__factory, "abi", _abi10);
...
var OftBridgeV0 = class extends OftBridgeBase {
  ...
  validateConfig(config) {
    uiCore.assert(config.version === 0, "Invalid config.version: is not 0");
  ...
  async getMessageFee({
    ...
    const srcNative = uiCore.getNativeCurrency(srcChainKey);
    const contract = this.getContractV0(srcChainKey);
    const dstEid = this.chainKeyToEndpointId(dstToken.chainKey);
    const dstAddress = contract.address;
    const dstNativeAmount = adapterParams.dstNativeAmount ? adapterParams.dstNativeAmount.quotient : ethers.BigNumber.from(0);
    ...
    const nativeFee = uiCore.CurrencyAmount.fromRawAmount(srcNative, fee.nativeFee.toBigInt());
    return {
      nativeFee,
  ...
  async getOutput({
    ...
    const outputAmount = uiCore.castCurrencyAmountUnsafe(srcAmount, dstToken);
    return {
  ...
  getContractV0(chainKey) {
    ...
    uiCore.assert(oft, `No oft for chainKey: ${chainKey}`);
...
var OftBridgeV1 = class extends OftBridgeBase {
  ...
  validateConfig(config) {
    uiCore.assert(config.version === 1, "Invalid config.version: is not 1");
    uiCore.assert(config.fee === false, "Invalid config.fee: is not false");
  }
  async transfer(input) {
    this.validateInput(input);
  ...
  async getOutput({
    ...
    const outputAmount = uiCore.castCurrencyAmountUnsafe(srcAmount.subtract(fee), dstToken);
    return {
      fee: { bridgeFee: fee },
      dstAmount: outputAmount
    };
...
var OftBridgeV2Base = class extends OftBridgeBase {
  ...
  validateConfig(config) {
    uiCore.assert(config.sharedDecimals, "Invalid config.sharedDecimals: not provided");
    uiCore.assert(config.version === 2, "Invalid config.version: is not 2");
  }
  ...
  removeDust(amount) {
    return uiCore.removeDust(amount, this.config.sharedDecimals);
  }
};
...
var OftBridgeV2 = class extends OftBridgeV2Base {
  ...
  async transfer(input) {
    ...
    const dstEid = this.chainKeyToEndpointId(dstChainKey);
    const dstAddress = ethers.utils.hexlify(uiEvm.addressToBytes32(input.dstAddress));
    const callParams = {
      adapterParams,
      refundAddress: input.srcAddress,
      zroPaymentAddress: uiEvm.AddressZero
    };
    const populatedTransaction = contract.populateTransaction.sendFrom(
      input.srcAddress,
      dstEid,
  ...
  async getMessageFee({
    ...
    const native = uiCore.getNativeCurrency(srcChainKey);
    const defaultFee = {
      nativeFee: uiCore.CurrencyAmount.fromRawAmount(native, BigInt(1e14)),
      // 0.0001 native token
  ...
  async getOutput({
    ...
    const outputAmount = uiCore.castCurrencyAmountUnsafe(swapAmount, dstToken);
    return {
      dstAmount: outputAmount,
      fee: {
        bridgeFee: zero
...
var POOL_TOKEN_CHAINS = [
  lzDefinitions.ChainKey.TRON,
  lzDefinitions.ChainKey.TRONDEV,
  lzDefinitions.ChainKey.TRON_TESTNET,
  lzDefinitions.ChainKey.CELO,
...
var OftBridgeV3 = class extends OftBridgeBase {
  ...
  async getOutput(input) {
    ...
    const dstAmountMin = input.dstAmountMin ?? uiCore.CurrencyAmount.fromBigInt(dstToken, BigInt(0));
    const adapterParams = input.adapterParams ?? uiCore.AdapterParams.forV1(0);
    const dstAddress = input.dstAddress ?? uiEvm.AddressZero;
    ...
    const outputAmountLD = uiCore.CurrencyAmount.fromBigInt(
      srcToken,
      oftReceipt.amountReceivedLD.toBigInt()
    );
    const outputAmountRD = uiCore.castCurrencyAmountUnsafe(outputAmountLD, dstToken);
    const sentAmountLD = uiCore.CurrencyAmount.fromBigInt(srcToken, oftReceipt.amountSentLD.toBigInt());
  ...
  async buildSendParams(input) {
    ...
    const dstChainKey = dstToken.chainKey;
    const dstEid = this.chainKeyToEndpointId(dstChainKey);
    const minAmount = uiCore.castCurrencyAmountUnsafe(input.dstAmountMin, srcAmount.token);
  ...
  async getLimit({ srcToken, dstToken }) {
    ...
    if (POOL_TOKEN_CHAINS.includes(dstChainKey) && POOL_TOKEN_CHAINS.includes(srcChainKey)) {
      ...
      return uiCore.CurrencyAmount.fromBigInt(srcToken, availCredits.toBigInt());
    }
    return uiCore.CurrencyAmount.fromRawAmount(srcToken, uiCore.MaxUint256);
  }
  ...
  getContract(srcChainKey, dstChainKey) {
    ...
    const provider = this.providerFactory(srcChainKey);
    if (!uiCore.isEvmChainKey(dstChainKey) && oftNative) {
    ...
    uiCore.assert(oft, `No oft for chainKey: ${srcChainKey}`);
...
var OftBridgeV2Fee = class extends OftBridgeV2Base {
  ...
  validateConfig(config) {
    super.validateConfig(config);
    uiCore.assert(config.fee === true, "Invalid config.fee: is not true");
  }
  async transfer(input) {
    this.validateInput(input);
    const srcChainKey = input.srcChainKey;
  ...
  async getMessageFee({
    ...
    const lzParams = uiEvm.serializeAdapterParams(adapterParams);
    const dstEid = this.chainKeyToEndpointId(dstChainKey);
    const dstAddress = uiEvm.AddressOne;
    const useZro = false;
    const amount = 0;
    const contract = this.getContractV2Fee(srcChainKey);
    const response = await contract.estimateSendFee(
      dstEid,
      uiEvm.addressToBytes32(dstAddress),
      amount,
      useZro,
      lzParams
    );
  ...
  async getBridgeFee(inputAmount, dstChainKey) {
    ...
    return uiCore.CurrencyAmount.fromRawAmount(swapAmount.token, fee.toBigInt());
  }
...
exports.OftBridgeApiFactory__evm = OftBridgeApiFactory__evm;
exports.OftBridgeBase = OftBridgeBase;
exports.OftBridgeV0 = OftBridgeV0;
exports.OftBridgeV1 = OftBridgeV1;
exports.OftBridgeV2 = OftBridgeV2;
exports.OftBridgeV2Fee = OftBridgeV2Fee;
exports.OftBridgeV3 = OftBridgeV3;
//# sourceMappingURL=out.js.map
//# sourceMappingURL=chunk-QZRBCK35.cjs.map