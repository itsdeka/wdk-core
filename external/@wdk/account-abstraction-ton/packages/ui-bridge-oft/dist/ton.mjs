import { fromTronAddress } from './chunk-V5ZIFKDM.mjs';
import './chunk-EJHQDNHP.mjs';
import { parseTonAddress, TonContractWrapper, UsdtMinter, buildTonTransferCell, getJettonAddressFromWallet, createTransaction, bigIntToAddress, computeTonUlnAddress, computeTonEndpointAddress, computeTonChannelAddress, addressToBigInt, computeTonUlnConnectionAddress } from '@layerzerolabs/ui-ton';
import { assert, CurrencyAmount, getNativeCurrency, isTronChainKey, MaxUint256, getMessageDuration, MessageFee, Fraction, isTonChainKey } from '@layerzerolabs/ui-core';
import { beginCell, toNano } from '@ton/core';
import { ChainKey } from '@layerzerolabs/lz-definitions';
import { generateBuildClass, generateDecodeClass, buildClass, decodeClass } from '@layerzerolabs/lz-ton-sdk-v2';
...
var oftBuildClass = generateBuildClass(tonObjects);
var oftDecodeClass = generateDecodeClass(tonObjects);
...
var OftBridgeV3__ton = class {
  ...
  feeCache = new memoryCache.Cache();
  ...
  async getGasAsserts() {
    const oftStorage = await this.client.provider(parseTonAddress(this.addressConfig.oftProxy)).get("getContractStorage", []);
  ...
  supportsTransfer(srcToken, dstToken) {
    ...
    return Boolean(
      isTonChainKey(srcToken.chainKey) && this.tryGetDeployment(srcToken.chainKey)?.token.equals(srcToken) && this.tryGetDeployment(dstToken.chainKey)?.token.equals(dstToken)
    );
  }
};
...
export { OftBridgeApiFactory__ton, OftBridgeV3__ton, ulnConfigKey };
//# sourceMappingURL=out.js.map
//# sourceMappingURL=ton.mjs.map