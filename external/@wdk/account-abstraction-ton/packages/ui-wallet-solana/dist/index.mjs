import { ChainType, waitFor, assert, tryGetNetworkByNativeChainId } from '@layerzerolabs/ui-core';
import { AbstractWallet } from '@layerzerolabs/ui-wallet';
import { WalletReadyState, BaseWalletAdapter } from '@solana/wallet-adapter-base';
...
var SolanaWallet = class extends AbstractWallet {
  ...
  autoConnect = async () => {
    ...
    try {
      await waitFor(() => this.adapter.readyState === WalletReadyState.Installed, { timeout: 3e3 });
      await this.connect();
    } catch (e) {
...
var TestWalletAdapter = class extends BaseWalletAdapter {
  name = "Test Wallet";
  url = "";
  ...
  async signTransaction(transaction2) {
    const isVersionedTransaction = transaction2 instanceof VersionedTransaction;
    if (!isVersionedTransaction)
      throw new Error("Only VersionedTransaction is supported.");
