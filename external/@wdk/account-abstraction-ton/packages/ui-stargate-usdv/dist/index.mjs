...
import { tokenSchema, Token, getNativeCurrency, CurrencyAmount, assert, isToken, Fraction, castCurrencyAmountUnsafe, isEvmChainKey, MaxUint256, removeDust } from '@layerzerolabs/ui-core';
...
var TYPE_1 = 1;
var TYPE_2 = 2;
var MaxUint128 = ethers.BigNumber.from("0xffffffffffffffffffffffffffffffff");
function optionsType1(_extraGas) {
  const extraGas = ethers.BigNumber.from(_extraGas);
  assert(extraGas.lte(MaxUint128), "extraGas should be less than MaxUint128");
...
function optionsType2(_extraGas, _dstNativeAmt, _dstNativeAddress) {
  ...
  assert(dstNativeAmt.lte(MaxUint128), "dstNativeAmt should be less than MaxUint128");
...
var BridgeRecolorBridgeApi__evm = class extends USDVBridgeBase {
  ...
  async getMessageFee({ srcToken, dstToken, adapterParams }) {
    ...
    const sendParamStruct = {
      ...
      minAmountLD: 1,
      // doesn't affect fee
      to: this.messaging.mapDstAddress(ONE_ADDRESS),
      // doesn't affect fee
  ...
  async getLimit({ srcToken, dstToken }) {
    ...
    const maxSwapAmountB = lpUsdvBalance.divide(
      new Fraction(1e4 + srcTokenConfig.rewardBps - srcTokenConfig.feeBps, 1e4)
    );
...
var serializedUSDVConfigSchema = z.object({
...
export { BridgeRecolorBridgeApi__evm, MessagingApi, MinterProxyBridgeApi__evm, MsgType, USDVBridgeApi__evm, createUSDVConfig, serializedUSDVConfigSchema, usdvMainnetConfig, usdvTestnetConfig };
