...
import { AbstractWallet, UserRejectedRequestError, SyncWalletError } from '@layerzerolabs/ui-wallet';
import { action, flow, makeObservable, runInAction, computed, observable } from 'mobx';
import { ChainType, tryGetNetworkByNativeChainId, getNetwork, assert, waitFor, getRpcs, getBlockExplorers, getNetworkByNativeChainId } from '@layerzerolabs/ui-core';
...
var EvmWallet = class extends AbstractWallet {
  ...
  handleAccountsChanged = action((accounts) => {
...
var EvmDebugWallet = class extends AbstractWallet {
  constructor(wallet) {
    super();
    this.wallet = wallet;
    makeObservable(this, {
...
var OKXWallet = class extends InjectedWallet {
  ...
  tryGetProvider() {
    ...
    if (okxwallet)
      return createEIP1193Provider(okxwallet);
    return void 0;
  }
};
var PhantomWallet = class extends InjectedWallet {
  ...
  tryGetProvider() {
    ...
    if (ethereum && ethereum[this.identityFlag])
      return createEIP1193Provider(ethereum);
    return void 0;
  }
};
  ...
  switchChain = flow(
    function* (chainKey) {
      ...
      try {
        ...
        yield this.switchEthereumChain(network);
        try {
          yield waitFor(() => this.chainKey === chainKey, { timeout: 3e3 });
        } catch {
...
export { BraveWallet, CoinbaseSmartWallet, CoinbaseWallet, CoreWallet, DETECT_INTERVAL, DETECT_TIMEOUT, DeFiWallet, EvmDebugWallet, EvmWallet, ExternalWallet, InjectedWallet, MetaMaskWallet, MipdWallet, OKXWallet, PhantomWallet, ProviderIdentityFlag, TahoWallet, WalletConnect, getEIP1193Provider, getSigner, isWalletMultiSig, normalizeChainId, numberToHex, patchEIP1193Provider };
