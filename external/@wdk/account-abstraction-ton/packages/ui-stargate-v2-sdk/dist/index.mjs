import { __publicField, TokenMessaging__factory, StargatePoolNative__factory, StargatePool__factory, StargateOFT__factory } from './chunk-GKGKCQPE.mjs';
import z, { z as z$1 } from 'zod';
import { Token, Coin, MessageFee, parseCurrencyAmount, AdapterParams, CurrencyAmount, castCurrencyAmountUnsafe, isNativeCurrency, MaxUint256, getNativeCurrency, assertToken } from '@layerzerolabs/ui-core';
import { createTransaction, ERC20__api, ONE_ADDRESS, Options, addressToBytes32 } from '@layerzerolabs/ui-evm';
...
var serializedStargateV2ConfigSchema = z.object({
...
var stargateV2ConfigSchema = serializedStargateV2ConfigSchema.transform((data) => {
  ...
  for (const chainKey in data.deployments) {
    ...
    config.deployments[chainKey] = {
      eid: deployment.eid,
      lpToken: deployment.lpToken && Token.from({
        chainKey,
      ...
      token: deployment.token.address ? Token.from({
        chainKey,
...
var bigIntSchema = z$1.string().transform((value) => {
  try {
    return BigInt(value);
  } catch (error) {
    return value;
...
var passengerSchema = z$1.object({
  sender: z$1.string(),
  receiver: z$1.string(),
  ...
  rideStatus: z$1.string(),
  txHash: z$1.string(),
  blockNumber: z$1.number(),
  timestamp: z$1.number(),
  nativeDrop: z$1.boolean(),
  passengerBytes: z$1.string()
});
var path = z$1.object({
  srcEid: z$1.number(),
  dstEid: z$1.number(),
  srcChainKey: z$1.string(),
  dstChainKey: z$1.string()
});
...
var queueSchema = path.extend({
  queue: z$1.object({
    currentBusParams: z$1.object({
      capacity: z$1.number()
    }),
    passengers: passengerSchema.array()
  })
});
var busQueueSchema = z$1.discriminatedUnion("inQueue", [
  queueSchema.extend({
    bus: z$1.undefined().optional(),
    inQueue: z$1.literal(true)
  }),
  busSchema.extend({
    queue: z$1.undefined().optional(),
    inQueue: z$1.literal(false)
  })
]);
var busDriveSettingsSchema = z$1.object({
  // expressed in ms
  maxWaitTime: z$1.number().transform((ms) => ({ seconds: ms / 1e3 })),
  passengersToDrive: z$1.number()
});
  ...
  toBytes() {
    if (this.sendMode === 0 /* TAXI */) {
      ...
      const passengerBytes = this.passengers.map(
        (p) => utils.solidityPack(["bytes32", "uint64"], [p.address, p.amount]).replace("0x", "")
      ).join("");
...
var StargateV2Bridge = class {
  constructor(config, getProvider, erc20 = new ERC20__api(getProvider)) {
    this.config = config;
  ...
  async transfer(input) {
    ...
    const value = isNativeCurrency(input.srcToken) ? input.srcAmount.add(input.fee.nativeFee).quotient : input.fee.nativeFee.quotient;
...
__publicField(OFTToken__factory, "abi", _abi);

export { ETH_MAINNET, ETH_SANDBOX, ETH_TESTNET, METH_MAINNET, METIS_MAINNET, MsgType, OFTToken__factory, OftCmd, STARGATE_V2_STAKING_MAINNET, STARGATE_V2_STAKING_SANDBOX, STARGATE_V2_STAKING_TESTNET, SendMode, StargateV2AssetId, StargateV2Bridge, StargateV2Bus, StargateV2BusDrivenError, StargateV2BusDstNativeAmountError, StargateV2NoPassengersError, TOKEN_MESSAGING_MAINNET, TOKEN_MESSAGING_SANDBOX, TOKEN_MESSAGING_TESTNET, USDC_MAINNET, USDC_SANDBOX, USDC_TESTNET, USDT_MAINNET, USDT_SANDBOX, USDT_TESTNET, createConfig };
