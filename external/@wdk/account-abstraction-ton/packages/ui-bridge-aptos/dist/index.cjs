...
function getDstAddressForQuote(chainKey) {
  ...
  if (uiCore.isAptosChainKey(chainKey)) {
    return uiAptos.fullAddress("0").toString();
  }
...
function toLD(token, amountSD, SD = 6) {
  const amount = uiCore.CurrencyAmount.fromRawAmount(token, amountSD);
...
var AptosBridgeV2__aptos = class {
  ...
  async getMinDstGas(path) {
    ...
    return uiAptos.getMinDstGas(client, accounts, uaAddress, dstEid, BigInt(1 /* SEND */));
  }
  async getMessageFee(path, {
    ...
    return uiCore.MessageFee.from(path.srcChainKey, {
      nativeFee,
      zroFee: 0n
    });
  }
  async getRoute(input) {
    ...
    const srcAmount = uiCore.removeDust(input.srcAmount, this.sharedDecimals);
    ...
    const srcAmountRD = uiCore.castCurrencyAmountUnsafe(srcAmount, input.dstToken);
  ...
  async transfer(input) {
    ...
    const unwrap = uiCore.isCoin(input.dstToken);
  ...
  supportsTransfer(srcToken, dstToken) {
    return uiCore.isAptosChainKey(srcToken.chainKey) && uiCore.isEvmChainKey(dstToken.chainKey) && isValidPair(srcToken, dstToken, this.config);
  }
...
var AptosBridgeV2__evm = class {
  constructor(config, getProvider) {
    this.config = config;
    this.getProvider = getProvider;
    this.erc20 = new uiEvm.ERC20__api(getProvider);
  }
  erc20;
  sharedDecimals = 6;
  async getDuration(input) {
  ...
  async getRoute(input) {
    ...
    const dstAmount = uiCore.castCurrencyAmountUnsafe(srcAmount, input.dstToken);
    const dstAmountMin = dstAmount;
    const route = {
      allowance,
      srcAddress: input.srcAddress,
  ...
  async getAllowance(input) {
    if (!uiCore.hasAddress(input.srcToken))
  ...
  async transfer(input) {
    ...
    const adapterParams = uiEvm.serializeAdapterParams(
      input.dstNativeAmount.equalTo(0) ? uiCore.AdapterParams.forV1(extraGas) : uiCore.AdapterParams.forV2({
        extraGas,
        dstNativeAddress: input.dstAddress,
        dstNativeAmount: input.dstNativeAmount
      })
    );
    const value = uiCore.hasAddress(input.srcToken) ? messageFee.nativeFee.toBigInt() : messageFee.nativeFee.add(input.srcAmount).toBigInt();
    const populatedTransaction = uiCore.hasAddress(input.srcToken) ? contract.populateTransaction.sendToAptos(
  ...
  supportsTransfer(srcToken, dstToken) {
    return uiCore.isEvmChainKey(srcToken.chainKey) && uiCore.isAptosChainKey(dstToken.chainKey) && isValidPair(srcToken, dstToken, this.config);
  }
  async getMinDstGas(path) {
    return 10000n;
  }
  async getMessageFee(path, {
    ...
    const callParams = { refundAddress: uiEvm.AddressOne, zroPaymentAddress: uiEvm.AddressOne };
    const contract = this.getBridgeContract(path.srcChainKey);
    const dstNative = uiCore.getNativeCurrency(path.dstChainKey);
    const extraGas = Number(minDstGas);
    const adapterParams = uiEvm.serializeAdapterParams(
      dstNativeAmount > 0 ? uiCore.AdapterParams.forV1(extraGas) : uiCore.AdapterParams.forV2({
        extraGas,
        dstNativeAmount: uiCore.CurrencyAmount.fromBigInt(dstNative, dstNativeAmount),
        dstNativeAddress
      })
    );
...
var serializedDeploymentSchema = z__default.default.object({
  ...
  bridge: z__default.default.object({
    address: z__default.default.string()
  })
});
var serializedAptosBridgeConfigSchema = z__default.default.object({
  deployments: z__default.default.record(serializedDeploymentSchema),
  tokens: z__default.default.record(
    z__default.default.enum([
      "USDC" /* USDC */,
      "BUSD" /* BUSD */,
      "WBTC" /* WBTC */,
      "USDT" /* USDT */,
    ...
    z__default.default.array(uiCore.currencySchema)
  )
});
...
var AptosClaimV2__aptos = class {
  ...
  async claim(input) {
    ...
    const bridgeAddress = getDeployment(input.token.chainKey, this.config).bridge.address;
    const tokenAddress = getBridgeCoinType(input.token);
    const ownerAddress = uiAptos.fullAddress(input.owner).hex();
    const entryFunctionPayload = claimCoinPayload(bridgeAddress, tokenAddress, ownerAddress);
    return uiAptos.createTransaction(entryFunctionPayload, { client });
  }
};

exports.AptosBridgeV2__aptos = AptosBridgeV2__aptos;
exports.AptosBridgeV2__evm = AptosBridgeV2__evm;
exports.AptosClaimV2__aptos = AptosClaimV2__aptos;
