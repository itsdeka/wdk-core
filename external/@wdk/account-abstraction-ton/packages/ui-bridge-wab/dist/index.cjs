...
var OriginalTokenBridge = class extends BaseTokenBridge {
  ...
  async getAllowance({ token, address }) {
    if (!uiCore.isToken(token)) {
      return uiCore.CurrencyAmount.fromRawAmount(token, ethers.constants.MaxUint256.toString());
    }
  ...
  async transferEth(input) {
    v1.validateInput(input);
    uiCore.assert(!uiCore.isToken(input.srcToken));
...
var WrappedTokenBridge = class extends BaseTokenBridge {
  ...
  async transferToken(input) {
    ...
    const callParams = this.buildCallParams(input);
    const adapterParams = this.buildLayerZeroTxParams(input.adapterParams);
    const unwrapWeth = uiCore.isToken(input.dstToken) ? false : true;
    const populatedTransaction = contract.populateTransaction.bridge(
      srcToken.address,
      dstEid,
      input.srcAmount.quotient,
  ...
  async getWeth(chainKey) {
    const deployment = this.getDeployment(chainKey);
    uiCore.assert(deployment.bridgeOriginal, `No bridgeOriginal for ${chainKey}`);
    ...
    return uiCore.Token.from({ chainKey, address: weth, decimals: 18, symbol: "WETH" });
  }
  async getLimit({ srcToken, dstToken }) {
    const dstChainKey = dstToken.chainKey;
    const dstDeployment = this.getDeployment(dstChainKey);
    uiCore.assert(dstDeployment.bridgeOriginal, `No bridgeOriginal for ${dstChainKey}`);
    const original = OriginalTokenBridge__factory.connect(
      dstDeployment.bridgeOriginal.address,
      this.providerFactory(dstChainKey)
    );
    const dstTokenAddress = (uiCore.isToken(dstToken) ? dstToken : await this.getWeth(dstChainKey)).address;
    ...
    return uiCore.CurrencyAmount.fromRawAmount(srcToken, tvlLD.toString());
  }
  getContract() {
    const provider = this.providerFactory(this.bridge.chainKey);
...
var bridgeSchema = z__default.default.object({
  address: z__default.default.string()
});
var wrappedAssetBridgeConfigSchema = z__default.default.object({
  version: z__default.default.number().or(z__default.default.string()),
  tokens: z__default.default.array(z__default.default.array(uiCore.currencySchema)),
...
function serializeWrappedAssetBridgeConfig(input) {
  const serialized = {
    deployments: {},
    tokens: input.tokens.map((tokenGroup) => tokenGroup.map((token) => uiCore.serializeCurrency(token))),
    version: input.version
  };
  for (const chainKey in input.deployments) {
    const deployment = input.deployments[chainKey];
...
exports.OriginalTokenBridge = OriginalTokenBridge;
exports.WrappedTokenBridge = WrappedTokenBridge;
exports.createWrappedAssetBridge = createWrappedAssetBridge;
exports.createWrappedAssetBridgeConfig = createWrappedAssetBridgeConfig;
exports.serializeWrappedAssetBridgeConfig = serializeWrappedAssetBridgeConfig;
exports.wrappedAssetBridgeConfigSchema = wrappedAssetBridgeConfigSchema;
