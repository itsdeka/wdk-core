import { RouterETH__factory, Pool__factory, StargateFeeLibrary__factory, Router__factory, StargateWidget__factory } from './chunk-ATRGEUFR.mjs';
import z from 'zod';
import { currencySchema, Token, isNativeCurrency, CurrencyAmount, MaxUint256, isEvmChainKey, assert, castCurrencyAmountUnsafe, getNativeCurrency, isToken } from '@layerzerolabs/ui-core';
import { validateInput } from '@layerzerolabs/ui-bridge-sdk/v1';
import { ERC20__api, createTransaction, ONE_ADDRESS } from '@layerzerolabs/ui-evm';
...
var serializedContractSchema = z.union([
  z.object({
    address: z.string()
  }),
  z.string()
...
var serializedStargateConfigSchema = z.object({
...
var stargateConfigSchema = serializedStargateConfigSchema.transform((input, ctx) => {
  ...
  for (const chainKey in input.deployments) {
    ...
    const deployment = {
      ...
      stargateToken: stargateToken && Token.from({
        symbol: "STG",
        decimals: 18,
...
var StargateV1Bridge = class {
  ...
  async getAllowance({ token, address }) {
    if (isNativeCurrency(token))
      return CurrencyAmount.fromBigInt(token, MaxUint256);
  ...
  supportsTransfer(srcToken, dstToken) {
    ...
    if (link.chainPath.weight === 0)
      return false;
    if (!isEvmChainKey(link.srcPool.chainKey))
      return false;
  ...
  lzTxParams(adapterParams) {
    const lzTxParams = {
      ...
      dstNativeAmount: adapterParams.dstNativeAmount ? adapterParams.dstNativeAmount.quotient : constants.Zero
    };
    return lzTxParams;
  }
  getFeeLibraryAddress = pMemoize(
    async (lpToken) => {
...
export { StargateV1Bridge, StargateV1Sdk, StargateV1WidgetBridge, getChainPaths, mainnet, testnet };
