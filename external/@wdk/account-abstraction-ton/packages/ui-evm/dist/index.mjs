import { ERC20__factory } from './chunk-VLOJPSA3.mjs';
import { secp256k1 } from './chunk-UUQMZ5OM.mjs';
import { getAddress, parseAccount, call, numberToHex, hexToBigInt, hexToNumber, assertRequest, serializeStateOverride, BaseError, formatTransactionRequest, extract, encodeFunctionData, getAbiItem, decodeFunctionResult, createBatchScheduler, RpcRequestError, getChainContractAddress, addressResolverAbi, universalResolverResolveAbi, toHex, trim, textResolverAbi, universalResolverReverseAbi, getCallError, TransactionNotFoundError, TransactionReceiptNotFoundError, multicall3Abi, AbiDecodingZeroDataError, RawContractError, getNodeError, UnknownNodeError, stringify, withResolvers, WaitForTransactionReceiptTimeoutError, AbiEventNotFoundError, formatAbiItem, toEventSelector, keccak256, toBytes, encodeAbiParameters, ContractFunctionZeroDataError, InternalRpcError, ContractFunctionRevertedError, ContractFunctionExecutionError, formatGwei, hexToBytes, bytesToHex, InvalidSerializableTransactionError, AbiEventSignatureNotFoundError, DecodeLogDataMismatch, DecodeLogTopicsMismatch, MethodNotSupportedRpcError, stringToHex, panicReasons, stringToBytes, concat, isHex, universalSignatureValidatorAbi, encodeDeployData, universalSignatureValidatorByteCode, hexToBool, isAddressEqual, CallExecutionError, concatHex, prettyPrint, formatEther, size, createCursor, AbiEventSignatureEmptyTopicsError, decodeAbiParameters, AbiDecodingDataSizeTooSmallError, PositionOutOfBoundsError, HttpRequestError, UnknownRpcError, UserRejectedRequestError, SwitchChainError, ChainDisconnectedError, ProviderDisconnectedError, UnsupportedProviderMethodError, UnauthorizedProviderError, JsonRpcVersionUnsupportedError, LimitExceededRpcError, TransactionRejectedRpcError, ResourceUnavailableRpcError, ResourceNotFoundRpcError, InvalidInputRpcError, InvalidParamsRpcError, MethodNotFoundRpcError, InvalidRequestRpcError, ParseRpcError, TimeoutError, integerRegex, isAddress, InvalidAddressError, bytesRegex, BytesSizeMismatchError, sliceHex, checksumAddress, LruMap, wrapConstructor, rotr, Hash, createView, aexists, toBytes2, aoutput } from './chunk-FJJ4XJPX.mjs';
import { __export } from './chunk-3M6IXPVX.mjs';
import { getNetworkByNativeChainId, ChainType, CurrencyAmount, createRpcMap, getNetwork, assert, tryGetNetwork, isCoin, isToken, isEvmAddress, isSolanaAddress, Token } from '@layerzerolabs/ui-core';
import { ethers, utils, BigNumber } from 'ethers';
import { providers } from '@0xsequence/multicall';
import memoize from 'micro-memoize';
import { JsonRpcBatchProvider, BaseProvider, Provider, Web3Provider } from '@ethersproject/providers';
...
function createTransaction(populatedTransaction, options) {
  ...
  async function signAndSubmitTransaction(signer) {
    ...
    if (chainKey === "taiko" && provider) {
      const minGasPrice = BigNumber.from(9e7);
...
var StaticJsonRpcBatchProvider = class extends JsonRpcBatchProvider {
  _onError;
  async detectNetwork() {
...
var logger = new Logger("0.0.1");
var FailoverProvider = class extends BaseProvider {
  maxAttempts = 3;
  providerConfigs;
  providerEntries;
...
var createMulticallProviderFactory = (factory, multicall2) => memoize(
  (chainKey) => {
    if (!multicall2) {
    ...
    return new providers.MulticallProvider(factory(chainKey), multicall2);
  },
  { maxSize: Number.POSITIVE_INFINITY }
);
var createFailoverProviderFactory = (rpcMap = createRpcMap(), options) => {
  return memoize(
    (chainKey) => {
...
var p_try_default = module.exports;
...
function optionsType1(_extraGas) {
  ...
  assert(extraGas.lte(MAX_UINT_128), "extraGas should be less than MAX_UINT_128");
...
function optionsType2(_extraGas, _dstNativeAmt, _dstNativeAddress) {
  ...
  assert(dstNativeAmt.lte(MAX_UINT_128), "dstNativeAmt should be less than MAX_UINT_128");
...
var Options = class {
  ...
  static fromOptions(optionsHex) {
    ...
    if (optionsType === 3 /* TYPE_3 */) {
      ...
      while (cursor < optionsBytes.byteLength) {
        ...
        cursor += 1;
        const size4 = ethers.BigNumber.from(optionsBytes.slice(cursor, cursor + 2)).toNumber();
        cursor += 2;
  ...
  addExecutorLzReceiveOption(gasLimit, nativeDrop = 0) {
    ...
    assert(gasLimitBN.lte(MAX_UINT_128), "gasLimit shouldn't be greater than MAX_UINT_128");
    const nativeDropBN = ethers.BigNumber.from(nativeDrop);
    assert(nativeDropBN.lte(MAX_UINT_128), "value shouldn't be greater than MAX_UINT_128");
  ...
  addExecutorNativeDropOption(nativeDrop, receiver) {
    ...
    assert(amountBN.lte(MAX_UINT_128), "nativeDrop shouldn't be greater than MAX_UINT_128");
    this.addOption(1 /* EXECUTOR */, {
      type: 2 /* NATIVE_DROP */,
  ...
  addExecutorComposeOption(index2, gasLimit, nativeDrop = 0) {
    ...
    assert(gasLimitBN.lte(MAX_UINT_128), "gasLimit shouldn't be greater than MAX_UINT_128");
    const nativeDropBN = ethers.BigNumber.from(nativeDrop);
    assert(nativeDropBN.lte(MAX_UINT_128), "nativeDrop shouldn't be greater than MAX_UINT_128");
    const option = nativeDropBN.gt(0) ? {
      ...
      type: 3 /* COMPOSE */,
      params: ethers.utils.solidityPack(["uint16", "uint128"], [index2, gasLimitBN])
    };
  ...
  decodeExecutorLzReceiveOption() {
    ...
    for (const option of options) {
      ...
      const gas = ethers.BigNumber.from(buffer2.subarray(0, 16)).toBigInt();
      if (buffer2.length === 16) {
        return { gas, value: ethers.BigNumber.from(0).toBigInt() };
      }
      const value = ethers.BigNumber.from(buffer2.subarray(16, 32)).toBigInt();
  ...
  decodeExecutorNativeDropOption() {
    ...
    const results = options.reduce(
      (acc, cur) => {
        ...
        const amount = ethers.BigNumber.from(buffer2.subarray(0, 16)).toBigInt();
        const receiver = ethers.utils.hexlify(buffer2.subarray(16, 48));
        if (acc[receiver]) {
  ...
  decodeExecutorComposeOption() {
    ...
    const results = options.reduce(
      (acc, cur) => {
        const buffer2 = Buffer.from(trim0x(cur.params), "hex");
        const index2 = ethers.BigNumber.from(buffer2.subarray(0, 2)).toNumber();
        const gas = ethers.BigNumber.from(buffer2.subarray(2, 18)).toBigInt();
        const value = (buffer2.length === 34 ? ethers.BigNumber.from(buffer2.subarray(18, 34)) : ethers.BigNumber.from(0)).toBigInt();
        if (acc[index2]) {
...
async function recoverPublicKey({ hash, signature }) {
  ...
  const { secp256k1: secp256k12 } = await import('./secp256k1-EF5A4IGD.mjs');
  const signature_ = (() => {
...
var Web3StaticProvider = class extends Web3Provider {
...
export { AddressOne, AddressZero, BalanceProvider__evm, ERC20__api, ExecutorOptionType, FailoverProvider, ONE_ADDRESS, OptionType, Options, ProviderRpcErrorCode, StaticJsonRpcBatchProvider, VerifierOptionType, WorkerId, ZERO_ADDRESS, addressToBytes32, createEstimateGas, createFailoverProviderFactory, createGetProvider, createGetPublicClient, createMulticallProviderFactory, createProvider, createTransaction, hexZeroPadTo32, multicallDeployments, optionsType1, optionsType2, randomWeighted, randomizeOrder, serializeAdapterParams, trim0x };
