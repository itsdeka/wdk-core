import { assert, isAptosChainKey, isToken, CurrencyAmount, isCurrency, getNativeCurrency } from '@layerzerolabs/ui-core';
import pMemoize from 'p-memoize';
import ExpiryMap from 'expiry-map';
import { HexString, BCS, ApiError } from 'aptos';
...
function buildAirdropAdapterParams(uaGas, airdropAmount, airdropAddress) {
  ...
  const params = [0, 2].concat(Array.from(convertUint64ToBytes(uaGas))).concat(Array.from(convertUint64ToBytes(airdropAmount))).concat(Array.from(HexString.ensure(airdropAddress).toUint8Array()));
...
function convertUint64ToBytes(number) {
  return BCS.bcsSerializeUint64(number).reverse();
}
