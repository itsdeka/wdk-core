...
// src/types.ts
var serializedWrapperConfig = z.object({
...
var serializedPartnerConfig = z.object({
  caller: z.string(),
  callerBps: z.number(),
  partnerId: z.number()
});
...
__publicField(OFT__factory, "abi", _abi);
var _abi2 = [
  {
    inputs: [
      {
...
__publicField(OFTWrapper__factory, "abi", _abi2);
var _abi3 = [
  {
    inputs: [
      {
...
__publicField(ProxyOFT__factory, "abi", _abi3);

// src/config/mainnet.ts
var mainnet = {
  deployments: {
    ethereum: { oftWrapper: { address: "0x86355F02119bdBC28ED6A4D5E0cA327Ca7730fFF" } },
    bsc: { oftWrapper: { address: "0x86355F02119bdBC28ED6A4D5E0cA327Ca7730fFF" } },
    avalanche: { oftWrapper: { address: "0x86355F02119bdBC28ED6A4D5E0cA327Ca7730fFF" } },
    polygon: { oftWrapper: { address: "0x86355F02119bdBC28ED6A4D5E0cA327Ca7730fFF" } },
    arbitrum: { oftWrapper: { address: "0x86355F02119bdBC28ED6A4D5E0cA327Ca7730fFF" } },
    optimism: { oftWrapper: { address: "0x86355F02119bdBC28ED6A4D5E0cA327Ca7730fFF" } },
    fantom: { oftWrapper: { address: "0x86355F02119bdBC28ED6A4D5E0cA327Ca7730fFF" } },
    zkevm: { oftWrapper: { address: "0x86355F02119bdBC28ED6A4D5E0cA327Ca7730fFF" } },
    moonbeam: { oftWrapper: { address: "0x86355F02119bdBC28ED6A4D5E0cA327Ca7730fFF" } },
    // Experimental:
    zksync: { oftWrapper: { address: "0xDAc7479e5F7c01CC59bbF7c1C4EDF5604ADA1FF2" } },
    // Supports sendNativeOFT and sendNativeOFTFeeV2
    metis: { oftWrapper: { address: "0x36d4686e19c052787D7f24E6913cEbC025714895" } },
    base: { oftWrapper: { address: "0x36d4686e19c052787D7f24E6913cEbC025714895" } },
    linea: { oftWrapper: { address: "0x36d4686e19c052787D7f24E6913cEbC025714895" } },
    kava: { oftWrapper: { address: "0x36d4686e19c052787D7f24E6913cEbC025714895" } },
    moonriver: { oftWrapper: { address: "0x36d4686e19c052787D7f24E6913cEbC025714895" } },
    celo: { oftWrapper: { address: "0x36d4686e19c052787D7f24E6913cEbC025714895" } },
    okx: { oftWrapper: { address: "0x36d4686e19c052787D7f24E6913cEbC025714895" } },
    klaytn: { oftWrapper: { address: "0x36d4686e19c052787D7f24E6913cEbC025714895" } },
    harmony: { oftWrapper: { address: "0x36d4686e19c052787D7f24E6913cEbC025714895" } },
    coredao: { oftWrapper: { address: "0x36d4686e19c052787D7f24E6913cEbC025714895" } },
    scroll: { oftWrapper: { address: "0x36d4686e19c052787D7f24E6913cEbC025714895" } },
    mantle: { oftWrapper: { address: "0x36d4686e19c052787D7f24E6913cEbC025714895" } },
    zora: { oftWrapper: { address: "0x36d4686e19c052787D7f24E6913cEbC025714895" } }
  }
};

// src/OftWrapperBridge__evm.ts
var OftWrapperBridge__evm = class {
  constructor(providerFactory, oft, partner, wrapperConfig = mainnet) {
  ...
  getOftDeployment(chainKey) {
    const deployment = this.tryGetOftDeployment(chainKey);
    uiCore.assert(deployment, `No deployment for ${chainKey}`);
    return deployment;
  }
  // returns wrapper deployment for given chain
  getWrapperDeployment(chainKey) {
    const deployment = this.wrapperConfig.deployments[chainKey];
    uiCore.assert(deployment, `No oftWrapper for ${chainKey}`);
  ...
  supportsRegister(token) {
    if (!uiCore.isEvmChainKey(token.chainKey))
      return false;
  ...
  async getDuration({ srcToken, dstToken }) {
    ...
    const address = oftProxy ? oftProxy.address : oftNative ? oftNative.address : uiCore.isToken(token) ? token.address : null;
    uiCore.assert(address, "address");
  ...
  async getAllowance({ token, address }) {
    ...
    if (this.isValidNative(token)) {
      return uiCore.CurrencyAmount.fromRawAmount(token, ethers.constants.MaxUint256.toBigInt());
    }
  ...
  supportsTransfer(srcToken, dstToken) {
    ...
    if (!this.isValidToken(srcToken))
      return false;
    if (!this.isValidToken(dstToken))
      return false;
    return true;
  }
  ...
  async sendOFTV2(input) {
    uiCore.assert(this.oft.version === 2);
    ...
    const minAmountLD = uiCore.castCurrencyAmountUnsafe(
    ...
    const value = this.isValidNative(input.srcToken) ? input.srcAmount.add(input.fee.nativeFee).quotient : input.fee.nativeFee.quotient;
    const method = this.getSendMethod(input.srcToken);
    if (method !== "sendOFTV2" && method !== "sendOFTFeeV2" && method !== "sendProxyOFTV2" && method !== "sendProxyOFTFeeV2" && method !== "sendNativeOFTFeeV2") {
  ...
  async sendOFTV1(input) {
    uiCore.assert(this.oft.version === 1);
    const srcChainKey = input.srcChainKey;
    const wrapperContract = this.getWrapperContract(input.srcToken);
    const oftContract = this.getOFTV2Contract(input.srcToken);
  ...
  async getMessageFee({ srcToken, dstToken, adapterParams }) {
    ...
    const dstAddress = this.encodeDstAddress(uiEvm.ONE_ADDRESS);
    const useZro = false;
    const amount = 0;
  ...
  async getExtraGas({ srcToken, dstToken }) {
    uiCore.assert(srcToken, "srcToken");
    ...
    const packetType = 0 /* PT_SEND */;
    const extraGas = await oftContract.minDstGasLookup(dstEid, packetType);
    const extraGasNumber = extraGas.toNumber();
    if (extraGasNumber > 0)
      return extraGasNumber;
    return dstChainKey === "arbitrum" ? 3e6 : dstChainKey === "aptos" || dstChainKey === "aptos-testnet" ? 1e4 : (
      // other evm
      25e4
    );
  }
  async getOutput({
    ...
    const outputAmountRD = uiCore.castCurrencyAmountUnsafe(outputAmountLD, dstToken);
    return {
      fee: {
  ...
  removeDust(amount) {
    if (!this.oft.sharedDecimals)
      return amount;
    return uiCore.removeDust(amount, this.oft.sharedDecimals);
  }
  getQuotedAddress({ chainKey }) {
    const { oftNative, oftProxy, token } = this.getOftDeployment(chainKey);
    if (oftNative)
      return oftNative.address;
    if (oftProxy)
      return oftProxy.address;
    uiCore.assert(uiCore.isToken(token));
    return token.address;
  }
  async getAmountAndFees(inputAmount) {
    ...
    return {
      callerFee: uiCore.CurrencyAmount.fromRawAmount(srcToken, response.callerFee.toBigInt()),
      wrapperFee: uiCore.CurrencyAmount.fromRawAmount(srcToken, response.wrapperFee.toBigInt()),
      // amount passed to OFT.sendFrom
      amount: uiCore.CurrencyAmount.fromRawAmount(srcToken, response.amount.toBigInt())
    };
  }
  async getOFTFee(inputAmount, dstChainKey) {
    const srcToken = inputAmount.token;
    if (!this.oft.fee) {
    ...
    return uiCore.CurrencyAmount.fromRawAmount(inputAmount.token, bridgeFee.toBigInt());
  }
  async getLimit({ srcToken, dstToken }) {
    return uiCore.CurrencyAmount.fromRawAmount(srcToken, uiCore.MaxUint256);
  }
  getWrapperContract(srcToken) {
  ...
  getOFTV2Contract(srcToken) {
    ...
    uiCore.assert(uiCore.isToken(srcToken));
    if (oftProxy) {
  ...
  buildLayerZeroTxParams(adapterParams) {
    return uiEvm.serializeAdapterParams(adapterParams);
  }
  getSendMethod(srcToken) {
    const { oftProxy, oftNative } = this.getOftDeployment(srcToken.chainKey);
    const fee = this.oft.fee;
    if (this.oft.version === 1) {
      if (oftNative)
        return "sendNativeOFT";
      if (oftProxy)
        return `sendProxyOFT`;
      return "sendOFT";
    }
    ...
    uiCore.assert(uiCore.isToken(srcToken));
    if (oftProxy && fee)
      return "sendProxyOFTFeeV2";
    if (oftProxy)
      return "sendProxyOFTV2";
    if (fee)
      return "sendOFTFeeV2";
    return "sendOFTV2";
  }
  buildLayerZeroCallParams(adapterParams, refundAddress, zroPaymentAddress = ethers.constants.AddressZero) {
    const callParams = {
  ...
  get feeObj() {
    return {
      ...
      partnerId: ethers.utils.hexZeroPad([this.partner.partnerId], 2)
    };
  }
  ...
  isValidToken(currency) {
    return Boolean(this.tryGetOftDeployment(currency.chainKey)?.token.equals(currency));
  }
};

exports.OftWrapperBridge__evm = OftWrapperBridge__evm;
exports.mainnet = mainnet;
exports.serializedPartnerConfig = serializedPartnerConfig;
exports.serializedWrapperConfig = serializedWrapperConfig;
